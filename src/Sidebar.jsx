...
const Sidebar = () => {
    ...
    return (
        <div
            className={`fixed top-16 right-0 h-[calc(100%-4rem)] bg-white shadow-md transition-all duration-300 z-40 ${isOpen ? "w-64" : "w-20"
                }`}>
            <button onClick={() => setIsOpen(!isOpen)}>
                <Bars3Icon className="w-6 h-6" />
            </button>
            {menuItem.map((item) => (
                <NavLink key={item.name} to={item.path} className={({ isActive }) => `flex items-center gap-3 px-3 py-6 ${isActive ? "bg-blue-50 text-blue-600" : "text-gray-700"
                    }`}>
